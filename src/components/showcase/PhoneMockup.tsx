...
export function PhoneMockup({ config, appName }: { config: DemoConfig; appName: string }) {
  ...
  return (
    <div>
      <PhoneFrame config={config} demoKey={demoKey} />
      ...
      {expanded ? (
        <div className="animate-fade-in fixed inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-foreground/40 p-4 backdrop-blur-md">
          <p className="text-xs font-medium text-primary-foreground">{appName}</p>
          <PhoneFrame config={config} demoKey={demoKey} fullscreen />
